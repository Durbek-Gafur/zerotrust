...
kubectl exec -it drone1 -- pkill iperf3
...
for i in {1..10}; do
  ...
  kubectl exec -it drone1 -- iperf3 -s &> /dev/null &
  pid_drone1_server=$!
  ...
  echo "Drone1 to Ground" | tee -a "$OUTPUT_FILE"
  kubectl exec -it drone1 -- iperf3 -c $RECEIVER_IP_GROUND &>> "$OUTPUT_FILE" &
  pid_ground=$!
  ...
  # Stop iperf3 server on drone1
  kubectl exec -it drone1 -- pkill iperf3
  wait $pid_drone1_server
