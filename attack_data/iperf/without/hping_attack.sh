...
for opt in "${HPING_OPTIONS[@]}"; do
  ...
  for i in {1..10}; do
    ...
    kubectl exec  drone4 -- iperf3 -c "$(kubectl get pod ground -o jsonpath='{.status.podIP}')" &>> "${OUTPUT_PREFIX}_${opt}.txt" &
    pid_ground=$!

    # Test from drone3 to drone4 to create congestion using hping3
    kubectl exec  drone3 -- hping3 "$(kubectl get pod drone4 -o jsonpath='{.status.podIP}')" -p 80 "--$opt" &>/dev/null &
    pid_drone4=$!
