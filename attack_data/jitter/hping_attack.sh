...
for opt in "${HPING_OPTIONS[@]}"; do
    for i in {1..10}; do
        ...
        kubectl exec -it drone1 -- sh -c 'ffmpeg -i input.mp4 -f mpegts udp://'"$RECEIVER_IP"':'"$VIDEO_PORT"' & sleep 10' &
        sleep 2

        # Start tcpdump on ground and save the output to a file
        OUTPUT_FILE="${PREFIX}_tcpdump_output_${opt}_${i}.pcap"
        kubectl exec -it ground -- timeout $DURATION tcpdump -s 0 -i $IFACE "udp port $VIDEO_PORT" -w $OUTPUT_FILE &
        pid_tcpdump=$!
        sleep 10
        
        # Start hping3 on drone3 to send packets to drone1
        kubectl exec -it drone3 -- hping3 "$(kubectl get pod drone1 -o jsonpath='{.status.podIP}')" -p $HPING_PORT "--$opt" &>/dev/null &
        pid_hping3=$!

        # Wait for the specified duration before stopping the tcpdump process
        sleep $DURATION

        # Stop tcpdump on ground
        kubectl exec -it ground -- pkill tcpdump
        wait $pid_tcpdump
        ...
        kubectl cp ground:$OUTPUT_FILE "${PREFIX}_tcpdump_output_${opt}_${i}.pcap"

        # Remove the output file from the ground pod
