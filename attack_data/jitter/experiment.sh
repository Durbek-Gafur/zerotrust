...
for opt in "${HPING_OPTIONS[@]}"; do
    for i in {1..3}; do
        ...
        kubectl exec  drone1 -- sh -c 'ffmpeg -i input.mp4 -f mpegts udp://'"$RECEIVER_IP"':'"$VIDEO_PORT"' >/dev/null 2>&1 &' &
        sleep 2

        # Start tcpdump on ground and save the output to a file
        OUTPUT_FILE="${PREFIX}_tcpdump_output_${opt}_${i}.pcap"
        kubectl exec  ground -- timeout $DURATION tcpdump -s 0 -i $IFACE "udp port $VIDEO_PORT" -w $OUTPUT_FILE &
        pid_tcpdump=$!
        sleep 10
        ...
        kubectl exec  ground -- pkill tcpdump
        wait $pid_tcpdump

        # Stop the video streaming on drone1
        kubectl exec  drone1 -- pkill ffmpeg
        ...
        kubectl exec  ground -- rm -f $OUTPUT_FILE
